{
  document.addEventListener("DOMContentLoaded", function() {
    ...
    // Set the dimensions as CSS variables
    ...
    // Adjust the width of the table and dropdown to match the slider
    const sliderWidth = baseWidth - 60;
    document.querySelector("table").style.width = `${sliderWidth}px`;
    document.getElementById("metricSelector-page2").style.width = `${sliderWidth}px`;
  
    // Append the SVG container for the treemap
    const svg = d3.select("#bar-chart-page2")
      .append("svg")
      .attr("width", sliderWidth) // Adjust width to fit within the container
      .attr("height", 300) // Fixed height to fit within the container
    ...
    d3.csv("specific_networks.csv").then((data) => {
      ...
      const sliderRange = d3
        ...
        .min(minYear)
        .max(maxYear)
        .width(sliderWidth - 20) // Adjust width to fit within the container
        .ticks(12)
      ...
      d3.select("#year-slider-page2")
        .append("svg")
        .attr("width", sliderWidth) // Extra width for padding
        .attr("height", 100)
        .append("g")
        .attr("transform", "translate(10,30)") // Center slider properly
        .call(sliderRange);
